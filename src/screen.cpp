	...
	scalingFactor(1)
{
	...
	if (!sdlInitErrorOccured) {
		window = SDL_CreateWindow("PacManNX",
...
TTF_Font *Screen::getSmallFont() {
	if (!smallFont)
		smallFont = loadFont("assets/fonts/Cheapmot.TTF", 12);
	return smallFont;
}
TTF_Font *Screen::getFont() {
	if (!font)
		font = loadFont("assets/fonts/Cheapmot.TTF", 20);
	return font;
}
TTF_Font *Screen::getLargeFont() {
	if (!largeFont)
		largeFont = loadFont("assets/fonts/Cheapmot.TTF", 24);
	return largeFont;
}
TTF_Font *Screen::getVeryLargeFont() {
	if (!veryLargeFont)
		veryLargeFont = loadFont("assets/fonts/Cheapmot.TTF", 48);
	return veryLargeFont;
}
TTF_Font *Screen::getSuperLargeFont() {
	if (!veryLargeFont)
		veryLargeFont = loadFont("assets/fonts/Cheapmot.TTF", 72);
	return veryLargeFont;
}
TTF_Font *Screen::getHugeFont() {
	if (!hugeFont)
		hugeFont = loadFont("assets/fonts/Cheapmot.TTF", 96);
	return hugeFont;
}
