...
MenuMain::MenuMain() {
		SDL_Surface *icon = Screen::loadImage("assets/gfx/pacman_desktop.png", 0);
		SDL_SetWindowIcon(Screen::getInstance()->getWindow(), icon);
		SDL_FreeSurface(icon);
		appTitle1 = Screen::getTextSurface(Screen::getSuperLargeFont(), "Pa", Constants::WHITE_COLOR);
		appTitle2 = Screen::getTextSurface(Screen::getSuperLargeFont(), "ManNX", Constants::WHITE_COLOR);
		titlePacman = Screen::loadImage("assets/gfx/title_pacman.png", 0);
...
void MenuMain::drawTitle() {
	...
	Screen::getInstance()->draw(appTitle1, rect.x, 50);
	rect.x = (short int) (rect.x + appTitle1->w + titlePacman->w);
	Screen::getInstance()->draw(appTitle2, rect.x, 50);
