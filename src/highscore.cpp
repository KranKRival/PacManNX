	...
	readonly(false)
{
	...
	//filePath = std::string(CommandLineOptions::getValue("", "highscore"));
	  // A path has been provided. If this path points to a directory, append default filename to it.
...
void HighscoreList::finishEntry() {
	if (entries->at(idxLastInsertedEntry)->getPlayerNameLength() == 0)
		entries->at(idxLastInsertedEntry)->setPlayerName("PacManNX");  // default name if none has been entered
